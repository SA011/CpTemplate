...
//        assert(a.size()+b.size()<=M);
    ...
    void multiply2D(vector<vector<T>> &a, vector<vector<T>> &b, vector<vector<T>> &res){
        ...
        for(int i = 0; i < a.size(); i++){
            for(int j = 0; j < a[i].size(); j++)v1[i][j].a = a[i][j];
        }
        for(int i = 0; i < b.size(); i++){
            for(int j = 0; j < b[i].size(); j++)v2[i][j].a = b[i][j];
        }
        fft2D(v1);
        fft2D(v2);
        for(int i = 0; i < Mxn; i++)
        ...
        for(int i = 0; i < Mxn; i++){
            res[i].resize(Mxm);
            for(int j = 0; j < Mxm; j++)res[i][j] = round(v1[i][j].a);
        }
    }
    void POWER(vector<ll> &a, ll n, vector<ll> &res){
        // call pre(possible maximum length) before this
        vector<cd> v1(M);
        for(int i = 0; i < a.size(); i++)v1[i].a = a[i];
        fft(v1);
