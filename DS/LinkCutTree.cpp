...
struct LinkCutTree{
    ...
    void propagate(int u){
        if(!u || !Nodes[u].lazy && !Nodes[u].rev)return;
    ...
    void combine(int u){
        ...
        if(Nodes[u].child[0])
            Nodes[u].sub += Nodes[Nodes[u].child[0]].sub, Nodes[u].sub_val += Nodes[Nodes[u].child[0]].sub_val;
        if(Nodes[u].child[1])
            Nodes[u].sub += Nodes[Nodes[u].child[1]].sub, Nodes[u].sub_val += Nodes[Nodes[u].child[1]].sub_val;
    }
    ...
    void link(int u, int v){        //usually u is parent. but it's not must
        access(v);
        access(u);
        if(!Nodes[v].child[0])setChild(v, 0, u);
        else if(!Nodes[u].child[0])setChild(u, 0, v);
        else{
            makeRoot(v);
    ...
    void cut(int u, int v){
        ...
        splay(v);
        int t = v;
        while(t != u && Nodes[t].child[1])t = Nodes[t].child[1];
        if(t == u)cut(u);
        else cut(v);
       //makeRoot(u);
       //access(u);
